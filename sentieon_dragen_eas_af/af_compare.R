...
sentieon_df <- read.delim(paste0(file_dir, "sentieon_", para, ".hg38_multianno_test.txt"), sep = "\t")
dragen_df <- read.delim(paste0(file_dir, "dragen_", para, ".hg38_multianno_test.txt"), sep = "\t")
...
scatter_plot <- function(input_df, x_column, y_column){
  ...
  subtitle_text <- paste0("Number of variants on ", para, ": ")
  scatter_plot <- ggplot(data = input_df, aes(x = as.numeric(input_df[[x_column]]), y = as.numeric(input_df[[y_column]]))) +
    ...
    subtitle = bquote(.(subtitle_text) ~ .(var_count) ~ ", " ~ r^2 ~ "=" ~ .(cor)))
  return(scatter_plot)
}

# Sentieon
## Remove AF_eas with "." or 0 (rm any missing values)
sentieon_df <- sentieon_df %>% filter(AF_eas != 0 & AF_eas != ".")
...
dragen_df <- dragen_df %>% filter(AF_eas != 0 & AF_eas != ".")
